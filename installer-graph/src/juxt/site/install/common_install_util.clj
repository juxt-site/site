...
(defn bundle-map [bundle uri-map opts]
  (if (or (#{"keep-old" "take-new"} (:on-find opts)) (not (:on-find opts)))
    (merge bundle {:uri (str (get-in uri-map ["https://data.example.org"]) "/bundles/" (clojure.string/replace (clojure.string/lower-case (:title bundle)) " " "-"))
                   :on-find (or (:on-find opts) "keep-old")
    ...
    (throw (ex-info "Invalid on-find option, choices are take-new, keep-old"
                    {:on-find (:on-find opts)
                     :title (:title bundle)}))))
